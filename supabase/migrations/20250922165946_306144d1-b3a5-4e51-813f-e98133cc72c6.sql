...
create policy if not exists "Anyone can upload temp files to poi-media"
...
create policy if not exists "Public can read poi-media"
on storage.objects for select
to anon, authenticated
using (bucket_id = 'poi-media');

-- Site owners can manage files under poi/<site_id>/* in poi-media
create policy if not exists "Site owners can insert into poi-media poi/{site_id}"
on storage.objects for insert
to authenticated
with check (
  bucket_id = 'poi-media'
...
create policy if not exists "Site owners can update their poi-media files"
...
create policy if not exists "Site owners can delete their poi-media files"
...
create policy if not exists "Owners can view their site media"
...
create policy if not exists "Owners can insert media for their sites"
...
create policy if not exists "Owners can update their media"
...
create policy if not exists "Owners can delete their media"
