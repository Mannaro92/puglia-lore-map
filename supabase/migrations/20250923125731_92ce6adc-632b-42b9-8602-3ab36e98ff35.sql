-- Create the RPC function to handle site upserts with proper coordinate handling
CREATE OR REPLACE FUNCTION public.rpc_upsert_site(
  p_site_id UUID DEFAULT NULL,
  p_payload JSONB,
  p_publish BOOLEAN DEFAULT FALSE,
  p_clear_geom BOOLEAN DEFAULT FALSE,
...
BEGIN
  ...
  ELSE
    ...
    SET
      ...
      fonte = COALESCE(p_payload->>'fonte', s.fonte),
...
-- Grant permissions
GRANT EXECUTE ON FUNCTION public.rpc_upsert_site(UUID, JSONB, BOOLEAN, BOOLEAN, UUID) TO anon, authenticated;

-- Add constraint to ensure published sites have coordinates
ALTER TABLE public.sites 
ADD CONSTRAINT sites_published_requires_geom 
CHECK (stato_validazione <> 'published' OR geom_point IS NOT NULL);