-- Update the rpc_upsert_site function to allow editing system POIs
...
BEGIN
    ...
    IF (payload->>'id') IS NOT NULL THEN
        ...
        -- Update main sites table - allow editing system POIs and user POIs
        UPDATE sites SET
            ...
            updated_at = now(),
            -- If it's a system POI being edited for the first time, assign ownership
            created_by = CASE 
                WHEN created_by = '00000000-0000-0000-0000-000000000000'::uuid OR created_by IS NULL 
                THEN auth.uid() 
                ELSE created_by 
            END
        WHERE id = site_uuid 
